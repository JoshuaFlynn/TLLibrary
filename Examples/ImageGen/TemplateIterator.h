...
A template iterator is merely an iterator front-end for TemplateList
Which allows you to have multiple access copies to a single TemplateList
(Beware race conditions! Programmers are advised to give nodes a boolean check or something)

This version allows you to modify the variables contained in the TemplateList
(Useful if you don't wish to modify what position the passed templatelist is at)

Note that the TemplateIterator offers no assurances of consistency if TemplateList's internal structure changes
Notably it will not have the correct size (size is non-real time) if increased/reduced
It will however tolerate node swaps - so long as the end and start nodes are not swapped
However concurrent access to TemplateIterator during a node swap is not assured
...
class TemplateIterator
{
	...
	public:
        ...
        const bool FindItem(const TemplateItem &ItemCopy, const bool FindBackwards = false)
        {
        ...
        const bool Find(const TemplateIterator &ItemCopy, const bool ReturnLast = false)
        {
            ...
            //This runs a 'top' heavy search for a single item
            //This primarily deals with arrays, rather than single items
        ...
        const bool Find(const TemplateItem &Item, const bool ReturnLast = false, const bool Debug = false)
        {
            return Find(&Item,1,ReturnLast,Debug);
        }

        const bool Find(const TemplateItem Data[], const SIZE_TYPE S, const bool ReturnLast = false, const bool Debug = false)
        {
            ERRORCLASSFUNCTION(TemplateIterator,Find,!IsValid(),RETURN_BOOL)
            ERRORCLASSFUNCTION(TemplateIterator,Find,!IsValidArray(Data,S),RETURN_BOOL)
            if(S > (Size-Iter+1)){return false;}
            TemplateIterator Temp1 = *this;
            const TemplateItem *TempStart = Data;
            const TemplateItem *TempEnd = (Data+S);

            //Searches to the first instance of the item if available
            while(Temp1.FindItem(*Data))
            {   
                //If found, are we looking for the last possible entry?
                if(ReturnLast)
                {
                    
                    do
                    {
                        if(Temp1.GetCurrItem() != *TempStart){break;}
                        ++TempStart;
                ...
                {
                    //Checks if the following is a match for it's position if we're comparing an array
                    if(Temp1.IsSubArray(Data,S))
                    {
                        Curr = Temp1.Curr;
                        Iter = Temp1.Iter;
                        return true;
        ...
        inline const TemplateItem &GetCurrPrevItem() const {return Curr->Prev->Item;}
        inline const TemplateItem &GetCurrNextItem() const {return Curr->Next->Item;}
