...
/*
Notes:
Modified float to string to work with file proc equals
...
template<typename TemplateItem>
const bool MatchJumbledArrays(const TemplateItem MatchTo[], const SIZE_TYPE ItemCopy, const TemplateItem Target[], const SIZE_TYPE ItemCopy1)
{
    ERRORFUNCTION(MatchJumbledArrays,!MatchTo,RETURN_BOOL)
    ERRORFUNCTION(MatchJumbledArrays,!Target,RETURN_BOOL)
    if(ItemCopy != ItemCopy1){ return false; }

    const TemplateItem * Original[ItemCopy];
    const TemplateItem * New[ItemCopy1];

    SIZE_TYPE Iter = 0;

    //We set up pointers-to-template to keep our memory overhead and CPU usage to a minimum
    while(Iter < ItemCopy)
    {
        Original[Iter] = MatchTo;
        New[Iter] = Target;
        Target++;
        MatchTo++;
        Iter++;
    ...
    //We begin looping through the first array
    while(Iter < ItemCopy)
    {
        Iter1 = 0;
        PassCheck = false;

        //And compare each instance of an 'item' in the first array to the second
        while(Iter1 < ItemCopy1)
        {
            //We make sure this item hasn't already been compared successfully
            if(New[Iter1] != NULL)
            {
                //If we get a successful item match...
                if( *(Original[Iter]) == *(New[Iter1]) )
                {
                    //We set the pointer to NULL so we know we've 'checked' it off
                    New[Iter1] =  NULL;
                    //Rather than using hideous code we'll absorb the overhead of a boolean
        ...
        //If PassCheck is false, we know we've looped through without finding a positive match
        if(!PassCheck)
        {
            return false;
        }
...
//From is the sub-array, MatchTo is the big array
template<typename TemplateItem>
const bool MatchJumbledSubArrays(const TemplateItem MatchTo[], const SIZE_TYPE ItemCopy, const TemplateItem From[], const SIZE_TYPE ItemCopy1)
{
    ERRORFUNCTION(MatchJumbledArrays,!MatchTo,RETURN_BOOL)
    ERRORFUNCTION(MatchJumbledArrays,!From,RETURN_BOOL)
    ERRORFUNCTION(MatchJumbledArrays,ItemCopy == 0,RETURN_BOOL)
    ERRORFUNCTION(MatchJumbledArrays,ItemCopy1 == 0,RETURN_BOOL)
    ...
    //We loop through the sub array first because this allows us to know when it's finished
    //And when no positive match returns
    ...
    while(Target < ItemCopy1)
    {
        Iter = 0;
        Succeeded = false;
        while(Iter < ItemCopy)
        {
            if(Original[Iter] != NULL)
            {
                if( *(Original[Iter]) == *(New[Target]) )
                {
                    //We set the pointer to NULL so we know we've 'checked' it off
                    New[Target] = NULL;
